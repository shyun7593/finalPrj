...
if (!$is_member)
    goto_url(G5_BBS_URL."/login.php?url=".urlencode(G5_SHOP_URL."/noticeView.php"));

// 읽지 않은 쪽지수
$memo_not_read = isset($member['mb_memo_cnt']) ? (int) $member['mb_memo_cnt'] : 0;

if (G5_IS_MOBILE) {
    include_once(G5_MSHOP_PATH.'/noticeView.php');
    return;
}

// 테마에 groupPage.php 있으면 include
if(defined('G5_THEME_SHOP_PATH')) {
    $theme_groupPage_file = G5_THEME_SHOP_PATH.'/noticeView.php';
    if(is_file($theme_groupPage_file)) {
        include_once($theme_groupPage_file);
        return;
        unset($theme_groupPage_file);
    }
}
