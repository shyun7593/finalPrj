...
if (!$is_member)
    goto_url(G5_BBS_URL."/login.php?url=".urlencode(G5_SHOP_URL."/college.php"));

// 읽지 않은 쪽지수
$memo_not_read = isset($member['mb_memo_cnt']) ? (int) $member['mb_memo_cnt'] : 0;

if (G5_IS_MOBILE) {
    include_once(G5_MSHOP_PATH.'/college.php');
    return;
}

// 테마에 college.php 있으면 include
if(defined('G5_THEME_SHOP_PATH')) {
    $theme_college_file = G5_THEME_SHOP_PATH.'/college.php';
    if(is_file($theme_college_file)) {
        include_once($theme_college_file);
        return;
        unset($theme_college_file);
    }
}
